...
int main (int argc, char** argv) {
  ...
  while(1) {
    ...
    /////////////////////////////////////////////
    matching_substring[0] == '\0';              // declare all the counters used
    int hash = 0;
    int i = 0;
    int isMatch = 0;
	...
	if (search_pattern[j] == input_string[temp]) {
	  matching_substring[k] = input_string[temp];    // this conditional checks for occurences of same letter
	  k++;
	  temp++;
	  j++;
	  ...
	  matching_substring[k] = input_string[temp];     // this conditional checks for the '.' metacharacter
	  k++;
	  temp++;
	  j++;
	  matching_substring[k] = '\0';
	  ...
	  while (input_string[temp] == search_pattern[j-1]) {  // this conditional checks for the '*' metacharacter
	  ...
	  temp++;
	  k++;                                              // this conditional checks for the fullstops
	  j = j + 2;
	  matching_substring[k] = '\0';
      ...
      if (k > 0) {
	if (hash == 1) {
	  print_char('#'); // prints hash in the middle if multiple substrings match
	}
	hash = 1;
    ...
    if (isMatch == 0) {                    // this is the no matches found counter
